...
class DTStat():
    ...
    def StdDev(self):
        ...
        if self.NumberOfObservations > 1.0:
            stddev = math.sqrt(
                (self.SumSquared - self.Sum ** 2 / self.NumberOfObservations) / (self.NumberOfObservations - 1))
        return stddev
...
class Entity():
    ...
    def __init__(self, Severity, NT, Status):
        ...
        self.Severity = Severity # patient's original severity
        self.NowTreatment = NT # Regular, ICU or None
        self.Status = Status # Alive, Unknown or Deceased in the end
        # For severity <= 3, 'Unknown' patients
        self.index = 0 # determine which phase the patient is: 0- before needing ICU; 1- after needing ICU
        self.HosTime = Clock # record time of ending first-phase regular
...
class FIFOQueue():
    ...
    def TopAdd(self, X):
        # Add an entity to the top of the queue
        self.ThisQueue.insert(0, X)
        numqueue = self.NumQueue()
        self.WIP.Record(float(numqueue))
